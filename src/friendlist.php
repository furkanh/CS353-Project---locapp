  ...
  if($table = $mysqli->query($sql)){
    if($table->num_rows>0){
      while($row = $table->fetch_row()){
        echo "<div class='container'><div class='row'><h4 class='display-10' ><a href='profile.php?uname=$row[0]'>$row[0]</a> <form method='post' action='message.php'><button type='submit' class='btn btn-primary' name=chatUsername value='$row[0]'>Send Message</button> </form></p></div></div>";
        echo "<hr class='my-4'>";
      }
    }
    else {
      echo "Get some friends.";
    }
  }
  $table->close();
?>
